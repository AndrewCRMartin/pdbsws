...
{
    print "\nINFO: *** Trying to update non-exact hits done > 7 days ago ***\n";
    DoUpdateProcessing();
...
sub DoMainProcessing
{
    ...
    foreach $pdbc (@pdbcs)
    {
        ...
        # If it is marked as already checked by BruteForceScan
        if(NeedsUpdate($pdb, $chain))
        {
            print "finding new SProt/trEMBL entries\n";
            $GotSeqs = BuildSprotFile($pdb, $chain, $::tmp2);
            if($GotSeqs)
            {
                $sprot = $::tmp2;
                BuildPDBChainSequenceFile($pdb, $chain, $::tmp1);
...
sub GetPDBChainList
{
    ...
    $sql = "SELECT pdb, chain FROM pdbsws WHERE valid = 'f'";
    $sth = $::dbh->prepare($sql);
    $rv = $sth->execute;
    while(@results = $sth->fetchrow_array)
    {
...
# BruteForceScan at least a month ago, but which have found non-exact hits
sub GetPDBChainListNonExact
{
    my($sql, $sth, $rv, @pdbcs);
    my(@results);
    
    $sql = "SELECT pdb, chain FROM pdbsws WHERE valid = 't' AND source = 'brute' AND (identity < 100.0 OR fracoverlap < 0.9) AND date < current_date - integer '30'";
    $sth = $::dbh->prepare($sql);
    $rv = $sth->execute;
    while(@results = $sth->fetchrow_array)
    {
